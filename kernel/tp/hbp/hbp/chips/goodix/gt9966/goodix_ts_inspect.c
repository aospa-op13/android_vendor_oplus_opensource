...
{
	if (ts_test->ts->ic_type == IC_TYPE_BERLIN_A)
		return (v1 - v2) * 63 / v2;
	else if (ts_test->ts->ic_type == IC_TYPE_BERLIN_B)
...
static void goodix_shortcircut_test(struct goodix_ts_test *ts_test)
{
    ...
    u16 test_time;
	u8 status;
	int ic_type = ts_test->ts->ic_type;
...
static int goodix_cap_test_prepare(struct goodix_ts_test *ts_test)
{
    ...
    /* switch rawdata mode */
	if (ts_test->ts->ic_type == IC_TYPE_BERLIN_D) {
		temp_cmd.cmd = 0x90;
...
static int goodix_cache_rawdata(struct goodix_ts_test *ts_test)
{
	...
	for (i = 0; i < TOTAL_FRAME_NUM; i++) {
		...
		if (cd->ic_type == IC_TYPE_BERLIN_D) {
